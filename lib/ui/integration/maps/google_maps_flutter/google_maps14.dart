...
import 'package:image/image.dart' as im_g;

class GoogleMaps14Page extends StatefulWidget {
  const GoogleMaps14Page({super.key});

  @override
  State<GoogleMaps14Page> createState() => _GoogleMaps14PageState();
}
...
class _GoogleMaps14PageState extends State<GoogleMaps14Page> {
  ...
  final double _iconFacebookSize=70;
  ...
  Future<ui.Image> _loadImage(img) async {
    final im_g.Image image = im_g.decodeImage(img)!;
    final im_g.Image resized = im_g.copyResize(image, width: _iconFacebookSize.toInt());
    final List<int> resizedBytes = im_g.encodePng(resized);
