...
class GoogleMaps4Page extends StatefulWidget {
  const GoogleMaps4Page({super.key});

  @override
  State<GoogleMaps4Page> createState() => _GoogleMaps4PageState();
}
...
class _GoogleMaps4PageState extends State<GoogleMaps4Page> {
  ...
  final List<LatLng> _positionList = [];
  ...
  @override
  void initState() {
    _positionList.add(const LatLng(-6.168033, 106.900467));
    _positionList.add(const LatLng(-6.164770, 106.900630));
    _positionList.add(const LatLng(-6.158637, 106.906376));
    super.initState();
  }
  ...
  */
  ...
  Widget _buildGoogleMap() {
    return GoogleMap(
          ...
          setState(() {
            ...
            for (int i = 0; i < _positionList.length; i++) {
              ...
              _allMarker[MarkerId(i.toString())] = Marker(
                ...
                onTap: () {
                  Fluttertoast.showToast(msg: 'Click marker ${i+1}', toastLength: Toast.LENGTH_SHORT);
                },
      ...
      onTap: (pos){
        //debugPrint('currentZoom : $_currentZoom');
      },
    );
  }
}
