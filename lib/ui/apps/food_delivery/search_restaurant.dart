...
import 'package:devkitflutter/model/apps/food_delivery/search_restaurant_model.dart';
  ...
  Widget _showHistorySearch() {
    return ListView(
      ...
      children: [
        _searchData.isEmpty
            ? const Wrap()
            : const Text('Last Search', style: GlobalStyle.lastSearchTitle),
        _searchData.isEmpty
            ? const Wrap()
            : Column(
        ...
        GestureDetector(
          ...
          child: Row(
            children: [
              const Icon(Icons.access_time, color: softGrey, size: 16),
              const SizedBox(width: 12),
              Text(_etSearch.text, style: const TextStyle(color: black55)),
            ],
          ),
        ),
      ],
